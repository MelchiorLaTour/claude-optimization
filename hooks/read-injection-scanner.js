#!/usr/bin/env node
// Read Injection Scanner — PostToolUse hook (global, standalone)
...
//
// Defense-in-depth: long sessions hit context compression, and the summariser
// does not distinguish user instructions from content read from external files.
// Poisoned instructions that survive compression become indistinguishable from
// trusted context. This hook warns at ingestion time.
//
// Fully self-contained (only Node `path`) — no dependency on any plugin/suite.
// Originated as gsd-read-injection-scanner.js; de-gsd-ified 2026-07-02
// (identity change only, scan logic unchanged).
//
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Real Read events nest it at tool_response.file.content (captured 2026-07-03).
    let content = '';
    const resp = data.tool_response;
    if (typeof resp === 'string') {
      content = resp;
    } else if (resp && typeof resp === 'object') {
      const c = resp.content ?? resp.file?.content;
      if (Array.isArray(c)) {
